...
class LOCUS(object):
    # Nm, Nr,  Nq
    def __init__(self, seq, record, sqt, k_size, w_size, g_size, tk, tp, tb, tm, tr, tq, pk_size, pw_size, pg_size, ptk,
                 ptp, ptb, ptm, ptr, ptq, visualisation, delete, option_mafft, option_blastn, option_blastp,
                 format_type):
        self.HRA_list = []
        self.Id = record.id
        self.seq = seq
        ...
        ratio = (head_seq.count("A") + head_seq.count("T") + head_seq.count("G") + head_seq.count("C") + head_seq.count(
            "N")) * 1.0 / len(head_seq)
    ...
    def cumulative_kmer_count(self):
        self.score_array, self.hra_range_list = kmer_count(self.seq, self.k_size, self.w_size, thresh=self.tk,
                                                           gap=self.g_size, buf=self.tm, seqtype=self.seqtype)
    ...
    def find_protein_HRA(self):
        ...
        for feat in self.record.features:
            if feat.type == "CDS" and "translation" in feat.qualifiers.keys():
                hra = HRA(i, "prot", self.pk_size, self.pw_size, self.pg_size, self.ptk, self.ptp, self.ptb, self.ptm,
                          self.ptq, self.ptr, self.option_mafft, self.option_blastn, self.option_blastp)
                region_score_array, phra_range_list = kmer_count(feat.qualifiers["translation"][0], self.pk_size,
                ...
                if len(phra_range_list) < 1:
                    ...
                    for j in range(len(phra_range_list)):
                        hra = HRA(i + j, "prot", self.pk_size, self.pw_size, self.pg_size, self.ptk, self.ptp, self.ptb,
                                  self.ptm, self.ptq, self.ptr, self.option_mafft, self.option_blastn,
                                  self.option_blastp)
                        ...
                        hra.region_seq = str(
                            feat.qualifiers["translation"][0][hra.hra_range[2]:hra.hra_range[3]]).upper()
    ...
    def find_HRA(self):
        ...
        for i, hra_range in enumerate(self.hra_range_list):
            if self.seqtype == "nucl":
                hra = HRA(i, "nucl", self.k_size, self.w_size, self.g_size, self.tk, self.tp, self.tb, self.tm, self.tq,
                          self.tr, self.option_mafft, self.option_blastn, self.option_blastp)
            else:
                hra = HRA(i, "prot", self.pk_size, self.pw_size, self.pg_size, self.ptk, self.ptp, self.ptb, self.ptm,
                          self.ptq, self.ptr, self.option_mafft, self.option_blastn, self.option_blastp)
                ...
                if len(feats) > 0:
                    for feat in feats:
                        if [feat.location.start, feat.location.end,
                            feat.qualifiers["periodicity_score"][0]] not in start_end_set:
                            spade_list.append(feat)
                            start_end_set.append(
                                [feat.location.start, feat.location.end, feat.qualifiers["periodicity_score"][0]])
                        else:
                            pass
                else:
                    rm_dir_list.append(os.getcwd())
...
class HRA(object):
    ...
    def kmer_period_matrix(self):
        self.period_matrix, self.region_score_array, self.kmer_position_dict = kmer_count_matrix(self.region_seq,
                                                                                                 self.k_size,
                                                                                                 self.w_size,
                                                                                                 seqtype=self.dtype)
    ...
    def search_peek(self):
        for kmer in self.kmer_position_dict:
            if len(self.kmer_position_dict[kmer][1]) > 1 and kmer != "":
                ...
                self.kmer_period_dict[kmer].extend(list(map(lambda x, y: x - y, self.kmer_position_dict[kmer][1][1:],
                                                            self.kmer_position_dict[kmer][1][0:-1])))

        # Position-Periodicity matrix integration with x axis
        self.sumx_matrix = np.sum(
            self.period_matrix[:, self.hra_range[0] - self.hra_range[2]:self.hra_range[1] - self.hra_range[2]],
            axis=1) / (self.hra_range[1] - self.hra_range[0])
    ...
    def check_periodicity(self, peak, limit, start, end):
        ...
        if region_sum == 0:
            ...
            self.periodicity = np.sum(
                self.period_matrix[peak - int(round(0.2 * peak)):peak + int(round(0.2 * peak)) + 1,
                start:end]) * 1.0 / region_sum
    ...
    def search_seed(self):
        ...
        for period in self.peak_period_set[:1]:
            if self.check_periodicity(period, 0.2, self.hra_range[0] - self.hra_range[2],
                                      self.hra_range[1] - self.hra_range[2]):
                max_count = 0
                seed_kmer = ""
    ...
    def get_unit_seq_list(self, seed_poss, period):
        ...
        for i, seed_pos in enumerate(seed_poss):
            ...
            #    unit_seq = gaps + self.region_seq[0:period-abs(r_num)]
                ...
                if i < len(seed_poss) - 1 and (
                        seed_poss[i + 1] - seed_pos < 1.2 * period and seed_poss[i + 1] - seed_pos > 0.8 * period):
    ...
    def output_data(self, clear=1):
        ...
        savetxt(f, np.concatenate(
            (np.array([list(range(p_min, p_max + 1))]).T, self.period_matrix[p_min:p_max + 1, :].astype(np.int)),
            axis=1), delimiter="\t", fmt=":.0f", header=header)
        #####
        #        Position
        # Pediod     1       2       3       4       5       6
        ...
        savetxt(f, np.concatenate(
            (np.array([list(range(p_min, p_max + 1))]).T, np.array([self.sumx_matrix[p_min:p_max + 1]]).T), axis=1),
                delimiter="\t", fmt=":.2f", header=header)
        #####
        # Period Periodicity
        # 1        score
        # 2        score
    ...
    def mafft(self, Exec=0):
        ...
        for period in self.peak_period_set:
            ...
            if self.dtype == "nucl":
                mafft_coms.append("mafft {} --quiet --auto {} > {}".format(self.option_mafft, fasta_name,
                                                                           fasta_name.replace("unit_seq.fasta",
                                                                                              "align.unit_seq.fasta")))
            else:
                mafft_coms.append("mafft {} --quiet --amino --auto {} > {}".format(self.option_mafft, fasta_name,
                                                                                   fasta_name.replace("unit_seq.fasta",
                                                                                                      "align.unit_seq.fasta")))
    ...
    def decide_query(self):
        ...
        for i, period in enumerate(self.peak_period_set[0:1]):
            ...
            if query_candidates[0][2] == 0 and query_candidates[-1][3] == len(frequent_seq) and len(
                    query_candidates) > 1:
                query_seq = frequent_seq[query_candidates[-1][2]:query_candidates[-1][3]] + frequent_seq[
                                                                                            query_candidates[0][2]:
                                                                                            query_candidates[0][3]]
            ...
            query.close()
            se_sets = [[pos + query_candidates[0][2], pos + query_candidates[0][2] + len(query_seq)] for pos in
                       self.unit_poss_list[i]]
        ...
        for i, period in enumerate(self.peak_period_set[0:1]):
            ...
            if len(self.query_list[i]) > self.k_size:
                ...
                if len(blast) > 2:
                    ...
                    fasta = open(fasta_name, "w")
                    blast = [elements for elements in blast if
                             float(elements[-2]) <= 0.001 or float(elements[6]) / float(elements[5]) >= 0.5]
                    consensus_motifs = []
                    true_motifs = []
                    pre_end = 0
                    ...
                    for n, consensus_motif in enumerate(consensus_motifs):
                        ...
                        aligned_positions.append(
                            [(int(blast[n][9]) - 1) - s, me - ms - (len(self.query_list[i]) - int(blast[n][10])) + e])
                        ...
                        if (int(blast[n][9]) - 1) - s <= 5:
                            ...
                            new_motif = consensus_motif[:(int(blast[n][9]) - 1) - s] + new_motif[
                                                                                       (int(blast[n][9]) - 1) - s:]
                    ...
                    for n, se_set in enumerate(se_sets):
                        if flag == 1:
                            ...
                            start = se_set[0]
                        if n < len(se_sets) - 1 and start + len(new_motifs[n]) - new_motifs[n].count("-") > \
                                se_sets[n + 1][0]:
                            flag = 1
                        else:
                            flag = 0
                    ...
                    for char_list in zip(*true_motifs):
                        ...
                        for char in char_set:
                            count_list.append(char_list.count(char))
                        # skip gap position
                        if char_set[count_list.index(max(count_list))] != "_" and char_set[
                            count_list.index(max(count_list))] != "-":
                            char = char_set[count_list.index(max(count_list))]
                            true_query += char
                            if max(count_list) * 1.0 / (
                                    sum(count_list) - char_list.count("_") - char_list.count("-")) < 0.6:
                                variable_query += "*"
                            else:
                                variable_query += char
                    fasta.close()
    ...
    def blast(self, Exec=0):
        ...
        for i, period in enumerate(self.peak_period_set):
            ...
            if self.dtype == "nucl":
                ...
                blast_com = "blastn -query {} -subject {} {} -out {}".format(query_name, subject_name,
                                                                             self.option_blastn, blast_name)
            else:
                ...
                blast_com = "blastp -query {} -subject {} {} -out {}".format(query_name, subject_name,
                                                                             self.option_blastp, blast_name)
    ...
    def make_motif_array(self):
        ...
        for i, period in enumerate(self.peak_period_set[:1]):
            ...
            p_min = period - max(int(round(0.2 * period)), 5) if period - max(int(round(0.2 * period)), 5) > 0 else 0
            p_max = period + max(int(round(0.2 * period)), 5) if period + max(int(round(0.2 * period)),
                                                                              5) < 1000 else 1000
            ...
            intensity = 0
            if period < 6:
                se_sets = [[se_set[0], se_set[1]] for se_set in se_sets if
                           np.sum(matrix[:, se_sets[0][0]:se_sets[-1][1]]) * 1.0 > 0]
            else:
                flag = 0
                new_se_sets = []
                for j, se_set in enumerate(se_sets[0:-1]):
                    if (period - int(round(0.2 * period)) < se_sets[j + 1][0] - se_set[0] and (
                            period + int(round(0.2 * period)) > se_sets[j + 1][0] - se_set[0])) or (
                            period - int(round(0.2 * period)) < se_sets[j + 1][1] - se_set[1] and (
                            period + int(round(0.2 * period)) > se_sets[j + 1][1] - se_set[1])):
            ...
            k = 0
            if len(se_sets) > 2:
                start = se_sets[0][0] if se_sets[0][0] >= self.hra_range[0] - self.hra_range[2] else self.hra_range[0] - \
                                                                                                     self.hra_range[2]
                end = se_sets[-1][1] if se_sets[-1][1] <= self.hra_range[1] - self.hra_range[2] else self.hra_range[1] - \
                                                                                                     self.hra_range[2]
                if start < 0:
                    start = 0
                ...
                if period < 6:
                    ...
                    intensity = np.sum(matrix[
                                       matrix.shape[0] // 2 - int(round(0.2 * period)):matrix.shape[0] // 2 + int(
                                           round(0.2 * period)) + 1, start:end], axis=1) * 1.0
                ...
                if periodicity < 0.5 and len(self.peak_period_set) > i + 1:
                    for add_period in self.peak_period_set[i + 1:]:
                        if add_period % period == 0 or period % add_period == 0:
                            p_min = add_period - max(int(round(0.2 * add_period)), 5) if period - max(
                                int(round(0.2 * add_period)), 5) > 0 else 0
                            p_max = add_period + max(int(round(0.2 * add_period)), 5) if period + max(
                                int(round(0.2 * add_period)), 5) < 1000 else 1000
                            add_matrix = self.period_matrix[p_min:p_max + 1, :].astype(np.int)
                            periodicity += np.sum(add_matrix[add_matrix.shape[0] / 2 - int(round(0.2 * add_period)):
                                                             add_matrix.shape[0] / 2 + int(round(0.2 * add_period)) + 1,
                                                  start:end]) / np.sum(score_array[start:end])
            else:
                periodicity = 0

            threash = self.tp
            if (periodicity >= threash) and len(se_sets) > 2 and (
                    np.argmax(intensity) == len(intensity) // 2 or period < 6):
    ...
    def make_feature(self, make_gb=True, make_tsv=True):
        ...
        for i, period in zip([0], self.peak_period_set):
            ...
            if self.dtype == "nucl" or self.feature == None:
                ...
                for se_set in se_sets:
                    if self.feature.strand == 1:
                        part_list_all.append([self.feature.location.start + 3 * (self.hra_range[2] + se_set[0]),
                                              self.feature.location.start + 3 * (self.hra_range[2] + se_set[1])])

                    else:
                        part_list_all.append([self.feature.location.end - 3 * (self.hra_range[2] + se_set[1]),
                                              self.feature.location.end - 3 * (self.hra_range[2] + se_set[0])])
            ...
            new_feat = SeqFeature(location, type="repeat_region",
                                  qualifiers={"dir_name": ["", ], "note": ["", ], "hrr_range": ["", ],
                                              "sequence_type": ["", ], "period": [], "rpt_type": [], "rpt_num": [],
                                              "periodicity_score": [], "rpt_unit_seq": [], "unmasked_rpt_unit_seq": []})
...
if __name__ == "__main__":
    ...
    p.add_argument("--mafft", type=str, default="--auto", help="Optional arguments for MAFFT can be defined with "
                                                               "single "
                                                               "quotations.\nDefault = '--auto'\nFor MAFFT "
                                                               "optional arguments, see "
                                                               "https://mafft.cbrc.jp/alignment/software/manual"
                                                               "/manual.html")
    p.add_argument("--blastn", metavar="<String>", type=str,
                   default='-strand plus -task blastn-short -penalty -2 -outfmt "6 qseqid qseq '
                           'sseqid sseq pident qlen length mismatch gapopen qstart qend sstart '
                           'send gaps evalue bitscore"', help="Optional arguments for BLAST+ "
                                                              "can be defined with single "
                                                              "quotations. Default = '-strand "
                                                              "plus -task blastn-short -penalty "
                                                              "-2 –outfmt '6 qseqid qseq sseqid "
                                                              "sseq pident qlen length mismatch "
                                                              "gapopen qstart qend sstart send "
                                                              "gaps evalue bitscore' blastp")
    p.add_argument("--blastp", metavar="<String>", type=str, default='-task blastp-short -outfmt "6 qseqid qseq '
                                                                     'sseqid sseq pident qlen '
                                                                     'length mismatch gapopen qstart qend sstart send '
                                                                     'gaps evalue '
                                                                     'bitscore"',
                   help="Optional arguments for BLAST+ can be defined with single quotations. Default = '-task "
                        "blastp-short –outfmt '6 qseqid qseq sseqid sseq pident qlen length mismatch gapopen qstart "
                        "qend sstart send gaps evalue bitscore' For BLAST+ optional arguments, "
                        "see https://www.ncbi.nlm.nih.gov/books/NBK279684/")
    p.add_argument("-n", "--num_threads", type=int, default=1, help="")
    p.add_argument("-v", metavar="<String, Permissible values: 'Y' 'N'>", type=str, default="Y", choices=("Y", "N"),
                   help="Generate pdf files to visualize results for each detected repeat region. Default = Y")
